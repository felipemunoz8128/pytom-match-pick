...
def predict_tophat_mask(
    ...
    """
    # convert float16 to float32 otherwise ndimage.white_tophat() fails and
    # score_volume.std() might produce np.inf
...
def extract_particles(
    ...
    if tomogram_mask is not None:
        ...
        if (
            tomogram_mask.dtype == "int8"
            and -128 <= tomogram_mask.min()
            and tomogram_mask.max() <= -127
        ):
